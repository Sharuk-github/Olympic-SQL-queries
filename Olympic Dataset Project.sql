...
-- 5. Which nation has participated in all of the olympic games?
SELECT region Nations FROM 
	(SELECT nr.region, COUNT(DISTINCT games) cnt FROM olympics_history oh
    JOIN olympics_history_noc_regions nr USING (noc)
	GROUP BY noc) x
WHERE x.cnt=51;
...
-- 6. Identify the sport which was played in all summer olympics.
SELECT sport FROM (	
	SELECT sport, ROW_NUMBER() OVER(PARTITION BY sport) rn FROM olympics_history 
    WHERE season = 'SUMmer'
	GROUP BY games , sport) y
WHERE rn= 29
ORDER BY rn;
...
-- 8. Fetch the total no of sports played in each olympic games.
SELECT games, COUNT(DISTINCT sport) 'No. of sports' FROM olympics_history
GROUP BY games;

-- 9. Fetch details of the oldest athletes to win a gold medal.
SELECT * FROM (
	SELECT *, MAX(age) OVER() oldest FROM olympics_history
	WHERE medal='Gold' AND age <> 'NA') x
WHERE age= oldest;
...
-- 11.	Fetch the top 5 athletes who have won the most gold medals.
SELECT name, total_gold FROM 
	(SELECT name, total_gold, DENSE_RANK() OVER(ORDER BY total_gold DESC ) rnk FROM
		(SELECT name, medal, COUNT(medal) OVER(PARTITION BY name) Total_gold 
		FROM olympics_history 
		WHERE medal= 'gold') x
	GROUP BY name) y
WHERE rnk<=5;
...
SELECT name, total_medals FROM 
	(SELECT name, total_medals, DENSE_RANK() OVER(ORDER BY total_medals DESC ) rnk FROM
		(SELECT name, medal, COUNT(medal) OVER(PARTITION BY name) total_medals 
		FROM olympics_history 
		WHERE medal <> 'NA') x
	GROUP BY name) y
WHERE rnk<=5;
...
-- 14. List down total gold, silver and broze medals won by each country.
WITH g AS (
		SELECT noc, COUNT(medal) Total_gold 
		FROM olympics_history 
		WHERE medal = 'gold'
		GROUP BY noc),
	s AS (
		SELECT noc, COUNT(medal) Total_silver 
		FROM olympics_history 
		WHERE medal = 'silver'
		GROUP BY noc),
	b AS (
		SELECT noc, COUNT(medal) Total_bronze
		FROM olympics_history 
		WHERE medal = 'bronze'
		GROUP BY noc),
	c AS (
		SELECT region, noc FROM olympics_history_noc_regions)
SELECT c.region, g.total_gold, s.total_silver, b.total_bronze
FROM c
JOIN g USING (noc)
JOIN s USING (noc)
JOIN b USING (noc)
ORDER BY g.total_gold DESC;
...
SELECT sport, event,
    SUM( CASE WHEN medal <> 'NA' then 1 else 0 end ) Total_medals
FROM olympics_history WHERE noc= 'IND' AND medal <> 'NA'
GROUP BY event
ORDER BY Total_medals DESC
LIMIT 1;
...
SELECT  games,
    SUM( CASE WHEN medal <> 'NA' then 1 else 0 end ) Total_medals
FROM olympics_history WHERE noc= 'IND' AND medal <> 'NA' AND sport = 'Hockey'
GROUP BY games
ORDER BY Total_medals DESC;
